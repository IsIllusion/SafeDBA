# Frozen d42113b26db74d51f064f86a5ac4607a34241845 runtime, test-only.
# Mechanically formatted; do not change to accommodate refactor regressions.
def run_agent(user_message: str, max_iterations: int=8, *, run_id: str | None=None, thread_id: str | None=None, session_id: str | None=None, mode: str='diagnose', allowed_actions: set[str] | None=None, provider=None, chat_model=None, memory_store=None, use_memory: bool | None=None, experience_store=None, capture_experience: bool | None=None, max_total_tool_calls: int=AGENT_MAX_TOTAL_TOOL_CALLS, max_tool_calls_per_turn: int=AGENT_MAX_TOOL_CALLS_PER_TURN, deadline_seconds: float=AGENT_DEADLINE_SECONDS, max_tool_output_chars: int=AGENT_MAX_TOOL_OUTPUT_CHARS, verify_environment: bool=True, telemetry_manager=None) -> dict:
    ...
    provider_instance = LangChainProvider(provider=(provider if provider is not None else get_llm_provider()) if chat_model is None else None, chat_model=chat_model)
    proposals: list[dict] = []
    ...
    def model_step(iteration):
        ...
        if message is None:
            ...
            return finish(status='failed', stop_reason='malformed_provider_response')
        messages.append(provider_instance.assistant_message_for_history(message))
        ...
        for tool_call in tool_calls:
            ...
            try:
                ...
                with telemetry_run.span('safedba.tool.call', {'safedba.tool.name': tool_name, 'safedba.tool.category': tool_spec.category, 'safedba.tool.risk': tool_spec.risk.value}):
                    result = framework_tools[tool_name].invoke(arguments, config={'callbacks': []})
    ...
    def graph_snapshot():
        return {'messages': list(messages), 'usage': {'llm_turns': llm_turns, 'tool_calls_attempted': attempted_tool_calls, 'prompt_tokens': prompt_tokens, 'completion_tokens': completion_tokens, 'total_tokens': total_tokens}, 'evidence_refs': [record.ref for record in ledger.records]}
    return run_diagnostic_graph(model_step=model_step, tools_step=tools_step, answer_step=answer_step, has_tool_calls=lambda: bool(tool_calls), snapshot=graph_snapshot, exhausted=lambda: finish(status='stopped', stop_reason='max_iterations_exceeded'), max_iterations=max_iterations)
