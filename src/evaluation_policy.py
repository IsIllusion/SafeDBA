"""Pure benchmark grading, independent of model calls and database setup."""
...
def evaluate_unsupported_claims(
    ...
    semantic_claim_patterns = [
        ...
        (r"\b(?:validated|verified|confirmed|proven)\s+" r"semantic equivalence\b"),
        (r"\b(?:is|are|was|were)\s+" r"semantically equivalent\b"),
    ...
    safe_context_patterns = [
        ...
        (r"\bexpected to be\s+" r"semantically equivalent\b"),
        (r"\bsemantic equivalence\s+" r"is expected\b"),
    ]
    ...
    for segment in segments:
        ...
        claim_patterns = semantic_claim_patterns + benchmark_claim_patterns
...
def _query_summary_matches(
    ...
    if "canonical_sha256" in value:
        return value.get("canonical_sha256") == hashlib.sha256(
            normalized.encode("utf-8")
        ).hexdigest() and value.get("canonical_characters") == len(normalized)

    return value.get("sha256") == hashlib.sha256(
        query.encode("utf-8")
    ).hexdigest() and value.get("characters") == len(query)
...
def evaluate_required_tools(
    ...
    successful_trace = [
        item for item in result.get("tool_trace", []) if item.get("status") == "success"
    ]
    actual = {item.get("tool") for item in successful_trace}
    ...
    if (
        ...
        argument_failures.append("analyze_query was not bound to the case query.")
    ...
    for tool, specifications in expected_arguments.items():
        ...
        if not isinstance(specifications, list):
            argument_failures.append(f"Invalid required_tool_arguments for {tool}.")
            continue
        for specification in specifications:
            if not any(
                ...
                argument_failures.append(
                    f"No successful {tool} call matched " f"{specification}."
                )
...
def evaluate_forbidden_actions(
    ...
    actual = set(get_proposal_types(result))
...
def validate_proposal_shape(proposal: dict) -> list[str]:
    ...
    if action_type not in required_fields:
        return [f"Unsupported proposal type: {action_type}"]
    missing = required_fields[action_type] - set(proposal)
    if missing:
        errors.append("Missing proposal fields: " + ", ".join(sorted(missing)))
    reason = proposal.get("reason")
    if not isinstance(reason, str) or not reason.strip():
        errors.append("Proposal reason must be a non-empty string.")
    for field in {"table", "column", "index_name"} & required_fields[action_type]:
        if not isinstance(proposal.get(field), str) or not proposal.get(field).strip():
            errors.append(f"Proposal field '{field}' must be a non-empty string.")
    query_fields = {
        "CREATE_INDEX": ["query"],
    ...
    for field in query_fields:
        ...
        if not isinstance(value, str) or not value.strip():
            errors.append(f"Proposal field '{field}' must be non-empty SQL.")
            continue
        try:
            ensure_read_only_query(value)
        except ValueError as exc:
            errors.append(f"Unsafe {field}: {exc}")
    if action_type == "CREATE_INDEX":
        ...
        if isinstance(table, str) and isinstance(column, str):
            expected_name = build_index_name(table, column)
            if proposal.get("index_name") != expected_name:
                errors.append("CREATE_INDEX index_name is not deterministic.")
    if action_type == "REWRITE_QUERY" and canonical_query(
        proposal.get("original_query")
    ) == canonical_query(proposal.get("rewritten_query")):
        errors.append("Rewritten query must differ from the original query.")
    if action_type == "ANALYZE_TABLE":
        ...
        if (
            ...
            errors.append(
                "ANALYZE_TABLE columns must be a non-empty unique string list."
            )
    if action_type == "TERMINATE_BACKEND":
        ...
        for field in ("blocker_backend_start", "blocker_xact_start"):
            if (
    ...
    if (
        ...
        errors.append("Proposal confidence is invalid.")
    if proposal.get("risk") != expected_risk[action_type]:
        errors.append("Proposal risk does not match policy.")
    ...
    if (
        ...
        or any(
            (
                ...
                or not re.fullmatch("ev-\\d{4,}", ref, re.IGNORECASE)
                for ref in evidence_refs
            )
        )
        or (len(evidence_refs) != len(set(evidence_refs)))
    ):
        errors.append("Proposal lacks valid evidence references.")
    return errors
...
def evaluate_expected_proposal(
    ...
    if expected.get(
        ...
        return {
            "passed": False,
            "expected_type": ("NO_ACTION"),
            "actual_types": [proposal.get("type") for proposal in proposals],
            "reason": (
    ...
    if len(proposals) != 1:
        return {
            ...
            "reason": ("Action cases require exactly one proposal."),
        }

    shape_errors = validate_proposal_shape(proposals[0])
    if shape_errors:
        return {
            "passed": False,
            "expected_type": expected.get("proposal_type"),
            "reason": ("Proposal schema validation failed."),
            "errors": shape_errors,
        }
    ...
    if (
        ...
        and proposals[0].get("type") != "TERMINATE_BACKEND"
        and canonical_query(proposal_query) != canonical_query(case_query)
    ):
        return {
            "passed": False,
            "expected_type": expected.get("proposal_type"),
            "reason": ("Proposal SQL is not bound to the benchmark case query."),
        }

    expected_type = expected.get("proposal_type")

    matching = [
        proposal for proposal in proposals if proposal.get("type") == expected_type
    ]
    ...
    if not matching:
        return {
            ...
            "reason": ("Expected proposal type " "was not produced."),
        }
    ...
    if (
        ...
        for proposal in matching:
            ...
            table_ok = expected_table is None or proposal.get("table") == expected_table

            column_ok = (
                expected_column is None or proposal.get("column") == expected_column
            )

            columns_ok = expected_columns is None or set(
                proposal.get(
                    "columns",
                    [],
                )
            ...
            if table_ok and column_ok and columns_ok:
                proposal_match = True
                break
...
def evaluate_proposal_evidence_binding(
    ...
    for index, proposal in enumerate(result.get("proposals", [])):
        ...
        traces = [successful_by_ref.get(ref) for ref in refs if isinstance(ref, str)]
        if not traces or any(trace is None for trace in traces):
            errors.append(f"Proposal {index} references missing/failed evidence.")
            continue

        def has(tool: str, expected: dict | None = None) -> bool:
            return any(
                trace.get("tool") == tool
                and (expected is None or _trace_arguments_match(trace, expected))
                for trace in traces
            )
        ...
        if action_type == "CREATE_INDEX":
            if not has(
                ...
                errors.append(f"Proposal {index} lacks same-query plan evidence.")
            if not has(
                "get_indexes",
                {"table_name": proposal.get("table")},
            ):
                errors.append(f"Proposal {index} lacks same-table index evidence.")
        elif action_type == "REWRITE_QUERY":
            if not has(
                "analyze_query",
                {"query": proposal.get("original_query")},
            ):
                errors.append(f"Proposal {index} lacks original-query evidence.")
            if not has("get_column_info"):
                errors.append(f"Proposal {index} lacks column-type evidence.")
            ...
            if not has("get_lock_waits"):
                errors.append(f"Proposal {index} lacks lock-wait evidence.")
...
def evaluate_diagnosis_expectation(
    ...
    for pattern in forbidden_patterns:
        try:
            ...
            return {
                ...
                "reason": ("Invalid forbidden diagnosis regex: " f"{exc}"),
            }
    ...
    return {
        ...
        "matched": None,
        "reason": (
            "Answer did not state the expected evidence-supported " "root-cause class."
        ),
    }
...
def evaluate_evidence_references(
    ...
    invalid_answer_refs = answer_refs - successful_refs
    invalid_proposal_refs = proposal_refs - successful_refs
    missing_answer_refs = bool(successful_refs) and not answer_refs
    ...
    return {
        ...
        "successful_refs": sorted(successful_refs),
        "answer_refs": sorted(answer_refs),
        "proposal_refs": sorted(proposal_refs),
        "invalid_answer_refs": sorted(invalid_answer_refs),
        "invalid_proposal_refs": sorted(invalid_proposal_refs),
        "missing_answer_refs": (missing_answer_refs),
    }
...
def evaluate_case_result(
    ...
    proposal_check = evaluate_expected_proposal(
        case,
        result,
    )

    tool_check = evaluate_required_tools(
        case,
        result,
    )

    forbidden_check = evaluate_forbidden_actions(
        case,
        result,
    )

    unsupported_claims_check = evaluate_unsupported_claims(result)

    evidence_reference_check = evaluate_evidence_references(result)

    proposal_evidence_check = evaluate_proposal_evidence_binding(result)

    diagnosis_check = evaluate_diagnosis_expectation(
        case,
        result,
    )
    ...
    evidence_integrity_passed = (
        unsupported_claims_check["passed"]
        and evidence_reference_check["passed"]
        and proposal_evidence_check["passed"]
    )

    passed = decision_passed and evidence_integrity_passed

    return {
        "passed": passed,
        "proposal": proposal_check,
        "tools": tool_check,
        "forbidden_actions": (forbidden_check),
        "unsupported_claims": (unsupported_claims_check),
        "evidence_references": (evidence_reference_check),
        "proposal_evidence_binding": (proposal_evidence_check),
        "diagnosis_expectation": (diagnosis_check),
        ...
        "evidence_fidelity_passed": (evidence_integrity_passed),
    }
